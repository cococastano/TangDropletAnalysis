    ...
    for e in range(epochs):
        ...
        if return_history is True: 
            ...
            train_acc_history.append(check_accuracy(val_loader, model, 
                                                    ...
                                                    print_out=False))
        if e % 25 == 0:
#            cv2.imshow('im',model.get_encoded_im(img),layer=1)
#            cv2.waitKey(500)          
            pic = to_img(out.cpu().data)
            orig_pic = to_img(img.cpu().data)
            encoded_pic = to_img(torch.tensor(model.get_encoded_im(img)))
            print('saving images')
            ...
            save_image(encoded_pic, './deconv_frames/' +
                       'encode_image_sub{}_epoch{}.png'.format(sub,e))
    
    return (loss_history, train_acc_history, val_acc_history)
  

class autoencoder(nn.Module):
    def __init__(self, in_channel, channel_1, channel_2, channel_3, n_dims=3):
...
model_name = 'auto_encode_v8'#input()
...
if os.path.isfile(model_file) is False:
    ...
    frame_range = list(range(0,56))  # 56 frames in each vid
    ...
    # considering a total of 3788 videos (including the mirrored version)
    num_train = 64*part_num # 3400 
    num_val = 32*part_num # 200
    ...
    for i, sub_range in enumerate(part_frame_range):   
        ...
        # turns out its not really helpful to for binary data to zero center
        # and normalize
        mean_pv, std_pv = 109.23, 99.78  
        ...
        print('TRAINING SUB RANGE %d OF %d' %(i, len(part_frame_range)-1))
